//! Command-prefix arities: how many tokens make up the "human-understandable
//! command" (`git checkout main` → `git checkout`). Flags never count.
...
fn lookup(prefix: &str) -> Option<usize> {
    ARITY.iter().find(|(k, _)| *k == prefix).map(|(_, v)| *v)
}

/// The command prefix tokens for a tokenized command line.
pub fn prefix(tokens: &[String]) -> Vec<String> {
    for len in (1..=tokens.len()).rev() {
        let key = tokens[..len].join(" ");
        if let Some(arity) = lookup(&key) {
...
mod tests {
    ...
    #[test]
    fn prefixes() {
        assert_eq!(prefix(&t("git checkout main")), t("git checkout"));
        assert_eq!(prefix(&t("npm run dev")), t("npm run dev"));
        assert_eq!(prefix(&t("npm install")), t("npm install"));
        assert_eq!(prefix(&t("touch foo.txt")), t("touch"));
